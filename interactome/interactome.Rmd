...
Next we select ligands and receptors from DE gene list
```{r}
l_c <- all_de_genes[rownames(all_de_genes) %in% pairs$ligand_symbol, c(6), drop = FALSE]
r_c <- all_de_genes[rownames(all_de_genes) %in% pairs$receptor_symbol, c(6), drop = FALSE]
```

And find indices of these gene's interactions in `all_ints_as` table
of interaction for Asbestos sample
```{r}
l_idx <- paste0(pairs$ligand_symbol[all_ints_as$lr_id], "/", all_ints_as$ligand_cl) %in% paste0(rownames(l_c), "/", l_c$cluster)
r_idx <- paste0(pairs$receptor_symbol[all_ints_as$lr_id], "/", all_ints_as$receptor_cl) %in% paste0(rownames(r_c), "/", r_c$cluster)
```

Finally, we select interactions from Asbestos sample for these indices,
cut them at 10% threshold and filter out from them any interactions
which are present in TiO2 sample at 10% threshold
```{r}
as_de_ints <- all_ints_as[l_idx | r_idx, ]
as_de_ints_0_1 <- as_de_ints[as_de_ints$int_exp > 0.1, ]
as_de_ints_0_1 <- as_de_ints_0_1[!(as_de_ints_0_1$id %in% all_ints_ti$id[all_ints_ti$int_exp > 0.1]), ]
```

```{r}
heatmap_interactions("SC15-SC14", as_de_ints_0_1) + 
    ggtitle(paste0(
        "Asbestos interactions, for which\n",
        "ligand or receptor is among DE genes\n",
        "and interaction is absent in TiO2\n",
        "with threshold = 10%"
    ))
```
...
write.csv(cbind(
    as_de_ints_0_1, 
    pairs[as_de_ints_0_1$lr_id, c(2, 4)]
